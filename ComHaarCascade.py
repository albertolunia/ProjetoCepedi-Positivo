...
def detectar():
    ...
    for z in imagemPath:
        ...
        for (x, y, l, a) in deteccoes:
            ...
            circles = cv2.HoughCircles(imgbb, cv2.HOUGH_GRADIENT, 1, 120, param1=100, param2=20, minRadius=10,
                                       maxRadius=20)
            if isinstance(circles, np.ndarray):
                ...
                for i in circles[0, :]:
                    ...
                    print(f"(x,y): ({x + i[0]},{y + i[1]})")
        cv2.imshow("Com Haar Cascade", img)
        cv2.waitKey(1000)
        cv2.imwrite('haar-placa-144307/' + str(numero) + '.png', img)
        numero += 1
